...
async function main() {
  console.log('Iniciando população de artigos sobre IA para iniciantes...');
  ...
  try {
    ...
    if (!category) {
      console.log('Criando categoria "IA para Iniciantes"...');
      ...
      console.log(`Categoria criada com ID: ${category.id}`);
    } else {
      console.log(`Categoria "IA para Iniciantes" já existe com ID: ${category.id}`);
    }
    ...
    if (!author) {
      console.log('Criando autor "Assistente IA"...');
      ...
      console.log(`Autor criado com ID: ${author.id}`);
    } else {
      console.log(`Autor "Assistente IA" já existe com ID: ${author.id}`);
    }
...
Deep Learning é um campo empolgante que continua a evoluir rapidamente. Embora possa parecer complexo no início, com os recursos certos e uma abordagem gradual, qualquer pessoa pode começar a explorar este mundo fascinante. O importante é começar com os conceitos básicos e ir progredindo para tópicos mais avançados à medida que sua compreensão aumenta.`,
    ...
    for (const topic of topics) {
      ...
      if (!existingArticle) {
        ...
        console.log(`Artigo criado com ID: ${article.id}`);
      } else {
        console.log(`Artigo já existe: ${topic.title}`);
      }
    }

    console.log(`População concluída. ${articles.length} artigos criados.`);
  } catch (error) {
